//===-- RArchISAInfo.h - RISC-V ISA Information -----------------*- C++ -*-===//
//
...
class RArchISAInfo {
public:
  RArchISAInfo(const RArchISAInfo &) = delete;
  RArchISAInfo &operator=(const RArchISAInfo &) = delete;
  ...
  RArchISAInfo(unsigned XLen, OrderedExtensionMap &Exts)
  ...
  static llvm::Expected<std::unique_ptr<RArchISAInfo>>
  parseArchString(StringRef Arch, bool IgnoreUnknown = false);
  ...
  static llvm::Expected<std::unique_ptr<RArchISAInfo>>
  postProcessAndChecking(std::unique_ptr<RArchISAInfo> &&ISAInfo);
  static std::string getTargetFeatureForExtension(StringRef Ext);

private:
  RArchISAInfo(unsigned XLen)
