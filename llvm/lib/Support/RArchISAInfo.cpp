//===-- RArchISAInfo.cpp - RISC-V Arch String Parser ------------*- C++ -*-===//
//
...
struct RArchSupportedExtension {
  const char *Name;
  /// Supported version.
  RArchISAInfo::ExtensionVersion Version;

  bool operator<(const RArchSupportedExtension &RHS) const {
...
static const RArchSupportedExtension SupportedExtensions[] = {
    {"a", {2, 1}},
    {"d", {2, 2}},
    {"i", {2, 1}},
    {"m", {2, 0}},
...
namespace {
struct LessExtName {
  bool operator()(const RArchSupportedExtension &LHS, StringRef RHS) {
    return StringRef(LHS.Name) < RHS;
  }
  bool operator()(StringRef LHS, const RArchSupportedExtension &RHS) {
    return LHS < StringRef(RHS.Name);
  }
};
} // namespace

static std::optional<RArchISAInfo::ExtensionVersion>
...
void RArchISAInfo::addExtension(StringRef ExtName,
                                 RArchISAInfo::ExtensionVersion Version) {
...
bool RArchISAInfo::isSupportedExtensionFeature(StringRef Ext) {
  ArrayRef<RArchSupportedExtension> ExtInfo =
...
bool RArchISAInfo::isSupportedExtension(StringRef Ext) {
  verifyTables();
...
bool RArchISAInfo::isSupportedExtension(StringRef Ext, unsigned MajorVersion,
...
bool RArchISAInfo::hasExtension(StringRef Ext) const {
...
bool RArchISAInfo::compareExtension(const std::string &LHS,
...
std::vector<std::string> RArchISAInfo::toFeatures(bool AddAllExtensions,
  ...
  if (AddAllExtensions) {
    for (const RArchSupportedExtension &Ext : SupportedExtensions) {
...
static Error getExtensionVersion(StringRef Ext, StringRef In, unsigned &Major,
  ...
  if (RArchISAInfo::isSupportedExtension(Ext, Major, Minor))
    return Error::success();
...
llvm::Expected<std::unique_ptr<RArchISAInfo>>
RArchISAInfo::parseFeatures(unsigned XLen,
                             const std::vector<std::string> &Features) {
  assert(XLen == 64);
  std::unique_ptr<RArchISAInfo> ISAInfo(new RArchISAInfo(XLen));
  ...
  return RArchISAInfo::postProcessAndChecking(std::move(ISAInfo));
}

llvm::Expected<std::unique_ptr<RArchISAInfo>>
RArchISAInfo::parseNormalizedArchString(StringRef Arch) {
...
static Error processSingleLetterExtension(
    StringRef &RawExt,
    MapVector<std::string, RArchISAInfo::ExtensionVersion,
  ...
  if (IgnoreUnknown && !RArchISAInfo::isSupportedExtension(Name))
    return Error::success();
...
llvm::Expected<std::unique_ptr<RArchISAInfo>>
RArchISAInfo::parseArchString(StringRef Arch, bool IgnoreUnknown) {
  ...
  for (auto &SeenExtAndVers : SeenExtMap) {
    ...
    RArchISAInfo::ExtensionVersion ExtVers = SeenExtAndVers.second;

    if (!RArchISAInfo::isSupportedExtension(ExtName)) {
      if (ExtName.size() == 1) {
        return createStringError(errc::invalid_argument,
...
Error RArchISAInfo::checkDependency() {
  return Error::success();
}
...
void RArchISAInfo::updateImplication() {
...
void RArchISAInfo::updateMinVLen() {
  for (auto const &Ext : Exts) {
...
std::string RArchISAInfo::toString() const {
  std::string Buffer;
...
llvm::Expected<std::unique_ptr<RArchISAInfo>>
RArchISAInfo::postProcessAndChecking(std::unique_ptr<RArchISAInfo> &&ISAInfo) {
...
StringRef RArchISAInfo::computeDefaultABI() const {
  if (XLen == 64) {
    if (hasExtension("d"))
      return "lp64d";
